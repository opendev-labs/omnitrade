    ...
    <div className="w-full flex flex-col gap-4 lg:gap-6">
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-6 lg:gap-8">
          <span className="hidden sm:inline text-xs font-mono-data text-muted font-bold">STRE_LINK: OPTIMAL</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-px bg-white/5 border border-white/5 rounded-sm overflow-hidden shadow-2xl">
        ...
        <div className="bg-void p-5 lg:p-6 flex flex-col justify-between group hover:bg-ink transition-all duration-700 ease-[var(--easing)] min-h-[100px]">
          ...
          <div>
            <div className={`text-base font-bold mb-1 tracking-tight ${state.trend === TrendState.UP ? 'text-accent' : 'text-primary'}`}>{state.trend} MOMENTUM</div>
            <div className="text-xs text-muted font-mono-data uppercase font-bold tracking-widest">{state.volatility} VOL</div>
          </div>
        </div>
        ...
        <div className="bg-void p-5 lg:p-6 flex flex-col justify-between group hover:bg-ink transition-all duration-700 ease-[var(--easing)] min-h-[100px]">
          ...
          <div className="space-y-2 lg:space-y-3 overflow-y-auto no-scrollbar max-h-[120px]">
            {(state.rotation || []).slice(0, 3).map(s => (
              <div key={s.ticker} className="flex justify-between items-center text-xs font-mono-data font-bold">
                ...
                <span className={`${s.strength >= 0 ? 'text-accent' : 'text-red-500'}`}>{s.strength.toFixed(2)}%</span>
              </div>
            ))}
            {(!state.rotation || state.rotation.length === 0) && (
              <div className="text-xs text-muted">Awaiting stream...</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-void p-5 lg:p-6 flex flex-col justify-between group hover:bg-ink transition-all duration-700 ease-[var(--easing)] min-h-[100px]">
          ...
          <div className="text-xs lg:text-sm text-secondary font-bold tracking-tight uppercase">
            {state.correlation ? `STRESS: ${(state.correlation.stress_index * 100).toFixed(0)}%` : 'Analysing...'}
          </div>
          <div className="w-full h-1 bg-white/5 mt-3 overflow-hidden rounded-full">
            <div
              className="h-full bg-accent transition-all duration-1000 opacity-60"
              style={{ width: `${(state.correlation?.stress_index || 0.1) * 100}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className={`p-5 lg:p-6 flex flex-col justify-between transition-all duration-1000 ease-[var(--easing)] ${state.uncertainty === 'CRITICAL' ? 'bg-red-950/20' : 'bg-void'} min-h-[100px]`}>
          ...
          <div className="flex justify-between items-center">
            <span className={`text-xs lg:text-sm font-bold uppercase tracking-tight ${state.uncertainty === 'CRITICAL' ? 'text-red-500' : 'text-muted'}`}>{state.uncertainty} RISK</span>
